...
const emptySupplier = () => ({
  name: '', email: '', bankName: '', bankBSB: '', bankAccountNumber: '', bankSwift: '',
});

function SupplierModal({ supplier, onClose, onSave }) {
  const isNew = !supplier?.id;
  const [form, setForm] = useState(isNew ? emptySupplier() : { ...supplier });
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="card pad" style={{ width: 'min(560px, 96vw)', maxHeight: '92vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ margin: 0 }}>{isNew ? 'Add Supplier' : 'Edit Supplier'}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
          <div style={{ gridColumn: '1 / -1' }}>
            ...
            <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="Supplier name" style={inputStyle} />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={fieldLabel}>Email</label>
            <input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="accounts@supplier.com" style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <input value={form.bankName} onChange={e => set('bankName', e.target.value)} placeholder="Bank name" style={inputStyle} />
          </div>
          <div>
...
export default function SupplierDatabase() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'ap_suppliers'), orderBy('name'));
  ...
  const handleSave = async (form) => {
    const { id, ...data } = form;
    if (id) {
      await updateDoc(doc(db, 'ap_suppliers', id), { ...data, updatedAt: new Date().toISOString() });
    } else {
      await addDoc(collection(db, 'ap_suppliers'), { ...data, createdAt: new Date().toISOString() });
    }
  };
  ...
  const handleDelete = async (supplierId) => {
    ...
    if (!ok) return;
    await deleteDoc(doc(db, 'ap_suppliers', supplierId));
  };
  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h1 style={{ margin: 0 }}>Supplier Database</h1>
        <button className="btn" onClick={() => setEditing({})}>+ Add Supplier</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card pad" style={{ textAlign: 'center', color: 'var(--muted)' }}>
          {search
            ...
            : 'No suppliers yet. Add one manually or they will be created automatically when a supplier invoice is received.'}
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map(supplier => (
                ...
                >
                  ...
                  <td style={{ padding: '10px 12px', color: 'var(--muted)', fontSize: 13 }}>{supplier.email || '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: 13, color: 'var(--muted)' }}>
                    {supplier.bankName
                      ? `${supplier.bankName}${supplier.bankBSB ? ` (${supplier.bankBSB})` : ''}`
                      : '—'}
                  </td>
                  <td style={{ padding: '10px 12px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        ...
                        onClick={e => { e.stopPropagation(); setEditing(supplier); }}
                      >
                        Edit
                      </button>
                      <button
